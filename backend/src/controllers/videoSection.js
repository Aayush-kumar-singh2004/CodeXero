...
const getVideosByProblem = async (req, res) => {
  try {
    ...
    if (!videos || videos.length === 0) {
      return res.json({ 
        message: 'No videos found for this problem',
        videos: []
      });
    }
    ...
    const formattedVideos = videos.map(video => ({
      ...
      uploader: {
        name: `${video.userId.firstName} ${video.userId.lastName}`,
...
module.exports = {generateUploadSignature,saveVideoMetadata,deleteVideo,getVideosByProblem};